...
class Generate3DGraph(html.Div):
  ...
  def __init__(self,
    ...
    def update_figure(clickData, fig, click_count, camera_data):
      fig, click_count = self.__highlight_course_node(clickData, fig, click_count, camera_data)
    ...
    def store_camera_position(relayoutData):
      if relayoutData is not None:
        if "scene.camera" in relayoutData:
          self.last_camera_position = relayoutData["scene.camera"]
  ...
  def __create_course_trajectory(self,
    ...
    fig.update_layout(
      ...
      clickmode='event',
      title=dict(text=f"3D Course Graph for {track_name} of {course_name}", x=0.5, font=dict(size=30, family="Arial", color="black", weight="bold")),
      width=2000,
      height=1050
    )
  ...
  def __highlight_course_node(self, clickData, fig, click_count, camera_data):
    if clickData:
      sleep(0.5)
            ...
            for j in range(len(fig["data"])):
              if "customdata" in fig["data"][j] and "edge" in fig["data"][j]["customdata"][0]:
                ...
                if node_edge_custom_data == custom_data:
                  ...
                  if pre_or_coreq == "pre":
                    fig["data"][j]["line"]["color"] = self.color_for_prerequisites
                  else:
                    fig["data"][j]["line"]["color"] = self.color_for_corequisites
            break
      ...
      fig["layout"]["scene"]["camera"] = self.last_camera_position
